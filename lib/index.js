...
var comb = require("comb"),
    ...
    util = require('util'),
    assert = require("./extensions/assert"),
            ...
            comb.async.array(this._expects).forEach(function (expect) {
                switch (expect[0]) {
                    case "cookie":
                        ...
                        if (!val) {
                            assert.isDefined(cookie, "expected cookie " + key + " to be present");
                        } else {

                            if (comb.isHash(val)) {
                                assert.deepEqual(cookie, val, "expected cookie " + key + " to equal " + JSON.stringify(val));
                            } else {
                                assert.equal(cookie.value, val, "expected cookie " + key + " value to equal " + val);
                            }
                        }
                        break;
                    case "!cookie":
                        //testing for cookies
                        key = expect[1];
                        assert.isUndefinedOrNull(getCookie(key, this._jar), "expected cookie " + key + " to be null or undefined ");
                        break;
                    case "header":
                        //it is a header test
                        ...
                        if (header in headers) {
                            if (comb.isRexExp(val)) {
                                assert.isTrue(headers[header].match(val), 'Expected "' + expect[1] + '" matching ' + val + ', got "' + headers[header] + '"');
                            } else {
                                if (header === "location") {
                                    var actual = headers[header];
                                    try {
                                        assert.equal(actual, val, [
                                            'Expected', util.inspect(val), 'response body, got', util.inspect(body)
                                        ].join(" "));
                                    } catch (e) {
                                        try {
                                            assert.equal(actual, this._baseUrl + val);
                                        } catch (e) {
                                            assert.equal(actual, this._baseUrl.replace(/^http[s]?:/, "") + val);
                                        }
                                    }
                                } else {
                                    assert.equal(headers[header.toLowerCase()], val,[
                    ...
                    case "body":
                        ...
                        if (comb.isHash(val)) {
                            ...
                            assert.deepEqual(json, val, ['Expected', util.inspect(val), 'response body, got', util.inspect(json)].join(" "));
                            ...
                            assert.deepEqual(body, val, ['Expected', util.inspect(val), 'response body, got', util.inspect(body)].join(" "));
                        }
                        break;
                    case "status":
                        assert.equal(res.statusCode, expect[1], ["Expected response status code to be", expect[1], "got", res.statusCode].join(" "));
                        break;
